...
% \newcommand{\Packager}{Packager}
\newcommand{\phantterm}{phantom term}
\newcommand{\phantterms}{phantom terms}
\newcommand{\Phantterms}{Phantom terms}
\newcommand{\phanttype}{phantom type}
\newcommand{\phanttypes}{phantom types}
\newcommand{\Phanttypes}{Phantom types}
...
\newcommand{\FP}{\ensuremath{\mathcal{H}}}
...
\emph{\factory{}} and \emph{\phantterm{}} that lets the hierarchy developer
describe an actual
...
\begin{command} to declare a new factory and generating new mixin builders:
\begin{coqcode}
...
\begin{definition}[\FP{}, \phantterms{}]
A \phantterm{} \(\phi \in \FP{}\) is a notation which automatically finds
the mixin instances a factory depends on and takes as explicit arguments
the contents of the factory. E.g.
\begin{coqcode}
Notation #$\varphi$# T := (#$f$# T ..).
Notation #$\Phi$# T  #\(x_1\ \ldots\ x_n\)# := (mkf T .. #\(x_1\ \ldots\ x_n\)#).
Definition b : #$\varphi$# T := #$\Phi$# T #\(x_1\ \ldots\ x_n\)#
\end{coqcode}
\end{definition}
\textbf{Now, for robustness sake, every single explicit use of a factory \(f\) as a Coq term
  must be replaced by \(\varphi\) and every use of its constructor by
  \(\Phi\).} The conception of such notation relies on unification
with phantom types \cite{DBLP:conf/itp/MahboubiT13}.


\subsection{Inheritance is inclusion}
...
A coercion between structures can be defined as follows and consists of twofold: a coercion between \coq{axioms} records which destructs a class instance of the subclass and reconstructs it as a class instance of the superclass (line~\ref{demo4:coe:ag:monoid:class}), and a coercion between \coq{type} records (line~\ref{demo4:coe:ag:monoid:type}).
\begin{coqcode}
...
In order to introduce multiple inheritance, we define the semiring structure, which is another intermediate structure between monoids and rings, and introduces binary multiplication operator \coq{mul} and its identity element \coq{one}.
Since semirings and Abelian groups do not inherit from each other, semirings can be defined similarly as in Abelian groups as follows.
\begin{coqcode}
Elpi hb.declare_mixin SemiRing_of_Monoid A Monoid.axioms.
  Record axioms := { one : A; mul : A -> A -> A; (* 7 axioms are omitted. *) }.
Elpi hb.end.
