...
\marginpar[]{This is the README in latex. I think we should separate the
coq part from the math part, eg M is a set of record NAMES. Then, in the
implementation part, say that these records have that specific form}
...
\coq{Record m3 (A : Type) (p1 : m1 A) (p2 : m2 A p1) := { ... }.}
...
\begin{definition}[\(\depx \in \M{} \to \powerset\M{}\)]
\label{def:depx}
Given \(m \in \M{}\), \(\depx (m)\) is the set of all \mixins{} that
occur as parameters of \(m\).
\end{definition}

\begin{definition}[\(\dep \in \powerset{\M{}} \to \powerset\M{}\)]
\label{def:depxs}
\(
...
\begin{proposition}[]
\label{prop:dep:trclosed}
\depx{} and \dep{} are transitively closed since records in \M{}
are well typed.
\end{proposition}

\begin{proposition}[]
\label{prop:dep:dag}
 \dep{} described a DAG (directed acyclic graph) since Coq does not
 admit circular definitions.
\end{proposition}

\begin{definition}[\(\depone{} \in \M{} \to \powerset\M{}\)]
\label{def:dep}
\(\depone{}(m) = \dep{}(\{m\})\)
\end{definition}
...
\coq{Class c (A : Type) := { f1 : m1 A; ...; f3 : m3 A f1 f2; f4 : m2 A }.}
\end{definition}

\newcommand{\cdef}{\ensuremath{def}}

\begin{definition}[\(\cdef{} \in \C \to \powerset\M{}\)]
\label{def:def}
\(\cdef{}(c)\) is the set of \mixins{} mentioned in the fields of the \class{}.
\end{definition}

\begin{proposition}[]
\label{prop:def:depclosed}
 \(\forall c \in \C{},~ \dep{}(\cdef{}(c)) \subseteq \cdef{}(c)\)
\end{proposition}

\theoremstyle{definition}
\newtheorem{requirement}[theorem]{Requirement}

\begin{requirement}[\cdef{} is injective]
No two \classes{} contain the same set of \mixins{}.
\end{requirement}
...
Its first field is \coq{(sort : Type)} and its second field is
\coq{(class : c sort)} for some \coq{c} \(\in \C{}\).
\end{definition}

\begin{requirement}[\C{} and \Str{} are in bijection]
For each \class{} there is one and only one \structure{}.
\end{requirement}

\newcommand{\issubclass}{\ensuremath{sub}}
\newcommand{\subclass}{subclass}

\begin{definition}[\issubclass{} \(\in \C{}\times\C{}\), \subclass{}]
\label{def:subclass}
...
\begin{definition}[\(\join{} \in \C{}\times\C{} \to \C{}\)]
\label{def:join}
...
\newcommand{\from}{\ensuremath{from}}

\begin{definition}[\F{}, \factory{}]
\label{def:factory}
...
The set \F{} comes equipped with the following functions:
\begin{itemize}
\item\(\requires{} \in \F{} \to \powerset\M{}\)
\item\(\provides{} \in \F{} \to \powerset\M{}\)
\item\(\from{} \in \F{} \times \M{} \to Coq\)
\end{itemize}

\begin{requirement} validate the following properties:
\begin{itemize}
...
\item\( \forall f \in \F{},~\forall m \in \provides{}(f),~ \from{}(f,m) = t \) then t : f to m
\end{itemize}
\end{requirement}
