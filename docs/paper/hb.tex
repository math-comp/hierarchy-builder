...
\newminted[coqcode]{coq}{linenos=true,fontsize=\footnotesize,mathescape}
...
tree with binders. Coq-Elpi takes full adavantage of this capability by representing
...
one mixin \elpi{M} then it crafts a name for it \elpi{Name} (line \ref{craft:name}),
fetches its dependencies (line \ref{deps:fetch}) and
finds the (previous) record arguments hoding these mixins (line \ref{deps:satisfy}). For this reason
...
and process \elpi{ML} we postulate a nominal
\elpi{m} (line \ref{postulate:m}) representing the current mixin
(line \ref{postulate:satisfy}) and we replace \elpi{f} by \elpi{m} in
\elpi{Fields} writing \elpi{(Fields m)}.
In this way line \ref{deps:satisfy} will
