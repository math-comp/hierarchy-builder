...
\begin{definition}[\cdef{}, class definition]
  We call \(\cdef{}(c) \in \powerset\M{}\) is the set of \mixins{}
  ...
  records contains the same set of \mixin{} (disregarding the order of
...
ways a user may refer to a class is as an argument of \hbmixin{},
\hbfactory{} or \hbstructure{}.

\begin{definition}[\Str{} Structure]
A \structure{} \coq{s} \(\in \Str{}\) is a Coq record with no parameters
...
Since the monoid structure is the bottom of this hierarchy, its class record \coq{Monoid.axioms} consists only of the \mixin{} to build monoids from scratch; in contrast, the class record of Abelian groups consists of monoid and Abelian group \mixins{} as follows because Abelian groups inherit from monoids.
\begin{coqcode}
Module AbelianGroup.
...
End AbelianGroup.
\end{coqcode}
As in the monoid structure, we declare \coq{AbelianGroup.sort} as an implicit coercion and lift additive inverse \coq{opp} and Abelian group axioms.
Since Abelian groups inherit from monoids, we also declare an implicit coercion from Abelian groups to monoids.
A coercion between structures can be defined as follows and consists of twofold: a coercion between \coq{axioms} records which reconstructs a given class instance of the subclass as a class instance of the superclass (line~\ref{demo4:coe:ag:monoid:class}), and a coercion between \coq{type} records (line~\ref{demo4:coe:ag:monoid:type}).
\begin{coqcode}
...
The first example of inference with canonical structures is \(0 + 1\), which can also be written as a Coq term \coq{@add _ (@zero _) (@one _)}.
Here \coq{add} and \coq{zero}, and \coq{one} are respectively from monoids and semirings; thus, the left- and right-hand sides can be typed as follows:
\begin{coqcode}
#$?_1$# : Monoid.type #$\vdash$# @add #$?_1$# (@zero #$?_1$#) : Monoid.sort #$?_1$# -> Monoid.sort #$?_1$#,
#$?_2$# : SemiRing.type #$\vdash$# @one #$?_2$# : SemiRing.sort #$?_2$#,
\end{coqcode}
where \(?_1~\coq{: Monoid.type}\) and \(?_2~\coq{: SemiRing.type}\) represent unification variables.

Since Coq does not know how to unify \(\coq{Monoid.sort}~?_1\) and \(\coq{SemiRing.sort}~?_2\), we are missing a piece of information; thus we have to find a common type that makes it work.
This information comes from the hierarchy and its inheritance relation (Definition~\ref{def:subclass}).
The low level mechanism is the unifier that compares \(\coq{A.sort}~?_1~\coq{= B.sort}~?_2\) and has to instantiate \(?_1\) and \(?_2\) with the \emph{smallest} common substructure of \coq{A} and \coq{B}~\cite[Sect.~4]{KSdraft}.
By declaring \coq{SemiRing_to_Monoid : SemiRing.type -> Monoid.type} as canonical, Coq can be aware of that instantiating \(?_1\) with \(\coq{SemiRing_to_Monoid}~?_2\) is the canonical solution of the first example.
In general, all the inheritance coercion between structures should be declared as canonical to make this inference work.

The second example is \(- 1\), which can also be writen as \coq{@opp _ (@one _)}.
Here \coq{opp} and \coq{one} are respectively from Abelian groups and semirings...



Similarly, one can apply an algebraic theory to an instance (an example) of that structure, e.g., \(2 \times 3\) where \(2\) and \(3\) has type \(\mathbb{Z}\).
