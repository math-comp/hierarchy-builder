...
Indeed, these two commands first create a record \(f_{aux}\) with a
constructor \(F_{aux}\) and then create abbreviations \(f\) and \(F\)
that automatically fill the mixin arguments of \(f_{aux}\) and \(F_{aux}\)
respectively.
In our target language these abbreviations rely on triggering
special unification problems with phantom types, see
\cite[Section 7]{DBLP:conf/itp/MahboubiT13} for a detailed description.
...
intermediate structure between monoids and rings as depicted in figure \ref{fig:puzzle_v4}, and introduces binary
