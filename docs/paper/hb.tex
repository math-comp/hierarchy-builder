...
\title{Hierarchy Builder: algebraic~hierarchies made easy with Coq-Elpi} %TODO Please add
...
We begin by building our first ``hierarchy''. It features only two
structures: \coq{TYPE} (a carrier with no properties) and \coq{Ring}.
...
that is going to be the root of the hierarchy.
...
\begin{coqcode}
Print Ring.type. (* Record type := { sort : Type;  class : Ring.class_of sort } *) #\label{demo:theory:print:type}#
Check @add. (* forall R : Ring.type, R -> R -> R *)                #\label{demo:theory:check:zero}#
Check @add0r. (* forall R : Ring.type, left_id zero add *)   #\label{demo:theory:check:add0r}#
...
Line~\ref{demo:theory:print:type} prints the type for rings as forged by \HB{}.
It packs a carrier, called \coq{sort}, and the collection of operation and
properties, called \coq{class}.
Lines~\ref{demo:theory:check:zero} and~\ref{demo:theory:check:add0r}
show the type of two constants synthesized by \HB{} out of the hierarchy
declaration. Note that while the names of the constants come from the
the names of the mixin fields, their types differ.
In particular they are quantified over a \coq{Ring.type}, and not
a simple type as in the mixin. Moreover we evince that
the \coq{sort} projection is declared as a coercion and is automatically
...
Line~\ref{demo:theory:zaxioms} uses the \coq{Ring_of_TYPE.Axioms}
\emph{packager} to build an instance of the \coq{Ring} structure for \coq{Z}.
Line~\ref{demo:theory:zaxioms:canonical} registers that
ring instance as the canonical one on \coq{Z}.
From now on the theory of rings applies to
integers, as shown in lemma \coq{exercise}.
\marginpar{I sue Axiom and not Axioms\_ everywhere, I think we should name
...
structure \coq{AddComoid} of commutative monoids.
...
Line~\ref{demo2:mixin:ring} declares a mixin for a type \coq{A}. This
time \coq{A} is not a naked type, but rather a type for which the
the assets of a commutative monoid\marginpar{kill class\_of} are available.
Indeed the operation \coq{add} is in scope and can be used
together with new operations such as \coq{opp} to state properties,
e.g. \coq{addNr}.
Line~\ref{demo2:structure:ring} declares the ring structure.

With the new hierarchy as declared so far some code we previously wrote
works but not all of it. For example the simple proof of \coq{addr0} works
but the declaration of the canonical ring over the integers fails, if only
because we don't have a \coq{Ring_of_TYPE.Axiom} anymore.
% Similarly we had to
% change the previously existing declaration of the ring structure.

The problem is depicted in figure~\ref{fig:puzzle} where three versions
of the hierarchy are depicted, the current one being the one in the middle.
...
We can make out new hierarchy compatible with pre exisintg user code by
declaring a \coq{Ring_of_TYPE} factory, that, from the user point of view,
...
Elpi hb.declare_factory Ring_of_TYPE A.
  ...
  Definition to_Ring_of_AddComoid :=
    Ring_of_AddComoid.Axioms A _ _ _ (addNr _) (mulrA _) (mul1r _)
      (mulr1 _) (mulrDl _) (mulrDr _).
  Elpi hb.canonical A to_Ring_of_AddComoid.
Elpi hb.end.
\end{coqcode}

The construction \coq{to_AddComoid_of_TYPE}
explain how to build a \coq{AddComoid} structure out of the factory
axioms (named \coq{a}). This construction is registered as canonical for \coq{A},
...
an \coq{AddComoid}). In this simple example the two constructions are just
partitioning the contents of factory, but in practice they often involve
interactive proofs that the properties held by the factories imply the
ones held by the mixins characterizing the structures tha are built.

Once the factory is declared one can replace
\coq{Elpi hb.structure Ring AddComoid_of_TYPE.axioms Ring_of_AddComoid.axioms}
with \coq{Elpi hb.structure Ring Ring_of_TYPE.axioms}, since the two commands
are now equivalent.

The new version of the hierarchy is now backward compatible with the initial one.
Indeed the user can declared \coq{Z} to be an instance
of a ring using the \coq{Ring_of_TYPE.axioms} factory. The associated
builder is going to generate behind the scenese instances of the
...
structures. The command \coq{Elpi hb.canonical Z Z_ring_axioms} is going
make \coq{Z} an instance of both structures. Indeed the
proof of \coq{example} now requires \coq{Z} not only to be a ring,
but also a \coq{AddComoid} since, for example, \coq{addrA} is a
