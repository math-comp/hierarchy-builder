...
\coq{axioms}, that packs operations and properties.
...
type into a monoid, hence we chose the module name to be \coq{Monoid_of_Type}.
...
is followd by \coq{Monoid.class_of}\marginpar{should be axioms} that enriches
...
We use the \coq{Monoid_of_Type.Axioms_} \emph{\packager{}}
(line~\ref{demo:theory:z:monoid:axioms})
in order to build an instance of the \coq{Monoid}
structure for binary integers \coq{ZArith.BinInt.Z}.
...
the \coq{Ring_of_Monoid.Axioms_} \packager{}
...
because we don't have a \coq{Ring_of_Monoid.Axiom} \packager{} anymore.
...
the \coq{Ring_of_AbelianGroup.Axioms_} \packager{} that requires \coq{A} to be
...
of a ring using the \coq{Ring_of_Monoid.Axioms_} \packager{}.
