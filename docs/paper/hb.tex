...
\marginpar{I'd say that mu and f are functions having as arguments a type T, plus some mixins. f build a type while nu a function from f T ms to ms+1 (return type without saying it is a function is fishy)}
\begin{definition}[\factories{} \F{}, \mixinbuilders{} \B{} and \from{}]
  ...
  a \emph{\factory{}}, and which depends on the \mixins{}.  I.e.
\begin{coqcode}
...
\begin{proposition}[\( \M{} \subseteq \F{} \)] For all \(m \in \M{}\)
  ...
  and
  \(\from(m,m) = \coq{fun T p_1 .. p_n (x :}\ m\
  \coq{T p_1 .. p_n) => x)} \in \B{}.\)
\end{proposition}
...
\marginpar{primitives is new terminology. I'd say factories are part of the interface of the hierarchy, hence cannot change in their contents but can change in their requires(f) since they are mixins. To avoid breaking client code these arguments have to be inferred automatically.}
Since \mixins{} may change but \factories{} stay the same, primitives
used by the user should never rely on a particular set of \mixins{} as
arguments.  This occurs each time one builds a factory for which
\requires{} is non empty. In these situations \mixin{} arguments of a
factory constructor must always be inferred automatically. To this
end, the commands \hbmixin{} and \hbfactory{} both generate packagers
to replace the constructor of the factory.
...
\begin{coqcode}
Notation #$\phi$# T #\(x_1\ \ldots\ x_n\)# := (...).
Definition b : f T #$\ldots$# := #$\phi$# T #\(x_1\ \ldots\ x_n\)#
\end{coqcode}
\end{definition}
...
Cite \cite{affeldt:hal-02463336}
