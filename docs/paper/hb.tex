...
client code is even harder, according to our experience~\cite{KSdraft}.
...
Moreover we can declare \coq{Z} to be an instance
...
Maybe we should be explicit here and say
\begin{itemize}
\item you only mention axioms, what goes on behind the scenes is not your concern, all thethings to makeinference work are taken care by HB (actually by its compilation in terms of PC discipline)
\item structures: they never disappear, but their composition may change
\item factories: you don't care if they are primitive or not
\item item you always use packagers that are generated, they could be trivial
record constructors or not
\item operations and properties: you don't care to which structure they belong.
If you used them on X, they can be moved anywhere else as log as X is a substructure.\marginpar{nothing talks about that in HB}
\end{itemize}
In some sense the user
