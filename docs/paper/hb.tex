...
\newminted[elpicode]{elpi.py:ElpiLexer -x}{linenos=true,fontsize=\footnotesize}
\newmintinline[elpi]{elpi.py:ElpiLexer -x}{fontsize=\small}
\newminted[coqcode]{coq}{linenos=true,fontsize=\footnotesize}
...
\author{Kazuhiko Sakaguchi}{Tsukuba Univerity, Japan}{sakaguchi@coins.tsukuba.ac.jp}{}{}
...
\keywords{Algebraic Hierarchy, Packed Classes, Coq, Elpi}%TODO mandatory; please add comma-separated list of keywords
...
\newcommand{\hb}{Hierarchy Builder}
...
\begin{abstract}
In this paper we design and implement a high level language
to declare hierarchies of algebraic structures in Coq.
\end{abstract}

\section{Introduction}
Packed Classes bla bla Mathematical Components library.
Initially in \cite{DBLP:conf/itp/GonthierAABCGRMOBPRSTT13}
Later used in \cite{DBLP:conf/mpc/AffeldtNS19} and \cite{DBLP:journals/mics/BoldoLM15}
and MC analysis (paper? TR?)

Packed Classes offer good performances compared to unbundled classes
or telescopes and good expressiveness but are unwildy to master.
To declare a hierarchy one does not only need to know all the details, but
also to declare a coercions and CS instance in a number that is 
quadratic in the size... and also error prone... Moreover changes to an
...
hierarchies. This language introduces the notion of \emph{factory} that is key
...
an intro to packed classes
\cite{DBLP:conf/itp/MahboubiT13}
\cite{DBLP:conf/tphol/GarillotGMR09}
\cite{DBLP:journals/jfp/GonthierZND13}
...
tree with binders.\\
Coq-Elpi takes full adavantage of this capability by representing
Coq terms in HOAS style, reusing the binder of the programming language in order
to reprent Coq's ones. Here an excerpt of the data type of Coq terms:

\begin{elpicode}
kind term type.
type global gref -> term.                    % eg: nat, S, Nat.add ...
...
In this syntax the Coq term \verb+(fun x : nat => x x)+ becomes
\elpi{(fun (global ...) x\ app[x, x])} where \elpi{x\} binds \elpi{x} in
what follows (the body of the function). The type \elpi{gref} decorates the
name of global constants: inductive types, constructors and constants.
...
These names are opaque to Elpi, that can only use them via dedicated APIs.
For example the following API lets one declare a record (more generally an
inductive type) and returns its name. \elpi{external pred} documents a
built in predicate by annotattin with \elpi{i:} (for input) and \elpi{o:}
(for output) the type of the arguments.
...
We comment it while looking at the code that declares the structure record given the
name of the class and produces three terms: the structure and the two
projections.
...
The code up to line 10 amounts to the following Coq code, assuming
that the value of the argument \elpi{ClassName} is \coq+class_of+:

\begin{coqcode}
Record type : Type := Pack { sort : Type; class : class_of sort }.
\end{coqcode}

Note that the binder \elpi{s\} at line 5 lets one mention the first field
in the type of the second. The syntax \elpi{{{ Type }}} is a quotation: it lets
one use the sytnax of Coq to write an Elpi term of type \elpi{term}.
When a record is declared Coq synthesizes the projections
automatically: the API \elpi{coq.CS.canonical-projection} lets us find them.
Finally, this program makes no use of backtraking and signal it using the
\elpi{std.do!} builtin to declare that no choice point have to be kept.

In this simple case the number of fields, and hence binders, is fixed.
The class record has one field per mixin and each of them
...
constants using the \elpi{pi n\} operator and attach to the
...
current computation).
This feature is key to in the following snippet that synthesizes the
...
The second predicate recurses on the list of mixins. If the list contains
one mixin \elpi{M} then it crafts a name for it \elpi{Name} (line X), fetches its dependencies (line Y) and
finds the (previous) record arguments hoding these mixins (line XXX). For this reason
the input mixin list must be topologically sorted.
It the build the type of current field, being the mixin name applied
to the type (sort) and all its dependencies.
Note that the \elpi{Fields} variable is under the binder for \elpi{f} (the current field).
In order to recurse under than binder and process \elpi{ML} we postulate a nominal
\elpi{m} representing the current mixin (line Z). In this way line XXX will
be able to satisfy the dependencies of the following mixins in \elpi{ML} using
\elpi{m}


Being a descendent of Prolog, Elpi is based on rules (clauses) that
represent both the code of the program and preexisting knowledge, for
example about the \elpi{class} predicate links the name of a class,
the name of its structure and the list of mixins cmoposing the class,
in other words the predicate holds for all the classes part of the hierarchy.
Being made of clauses a program can be easily extended by adding extra
clauses to it, and being Prolog a homoiconic language its programs can easily
manipalte its own syntax and in particular syntesize new clauses.

The use of \elpi{=>} is just an occurrence of this. We use it for the DB
as well, eg dep1 before. See the code for declare\_mixin
...
Line XX crafs a clause \elpi{(dep1 GR Mix)} that is added to
"hierarchy.db", a set of clauses used by all the commands part of \hb{}.

\section{Benchmark}
if we can port ssralg, or not...

\section{Related work}
about packed classes: Garillot and co, say again what we improve.

about describing a hierarchy, Cyril you should really give it a read and compare
it with our language:
Theory Presentation Combinators (OConnor , Carette) \cite{CaretteCombinators}

about organizing knowledge (they are more focused on description rather than inference)
MMT (Kolhase , co) \cite{RABE20131}
\section{Conclusion}

we lower the barrier via a high level language and an implementation.

what can be used in other systems?

maybe we should generate v code? at least declare instance has to be in vanilla Coq, PR opened.
