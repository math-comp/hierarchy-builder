...
\newcommand{\mixinbuilder}{mixin builder}
\newcommand{\mixinbuilders}{mixin builders}
\newcommand{\factorybuilder}{factory builder}
\newcommand{\factorybuilders}{factory builders}
...
\newcommand{\MB}{\ensuremath{\mathcal{MB}}}
\newcommand{\FB}{\ensuremath{\mathcal{FB}}}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\HB{} language and compilation to Packed Classes}\label{sec:compilation}

\subsection{\HB{} terms}

We call \(\Lang{}\) the set of terms of Coq type theory. The terms handled by \HB{} are subdivided in six categories,
the mixins \(\M{}\), the factories \(\F{}\), the classes \(\C{}\), the structures
\(\Str{}\), factory builders \(\FB{}\), mixin builders \(\from{}(\cdot,\cdot)\).

Mixins, factories and builders are tagged by the user, while
structures and classes are generated by \HB{}.
...
\begin{coqcode}
Record m (T : Type) (p_1 : m_1 T) (p_2 : m_2 T p_1) .. 
  (p_n : m_n T p_i1 .. p_ik) := { ... }.
\end{coqcode}
for \coq{m_1}, \coq{m_2}, .. and \coq{m_n} \(\in \M{}\).

Given \(m \in \M{}\), we define \(\depone{}(m) \in \powerset\M{}\) as
the set of all \mixins{} that occur as parameters of \(m\), eg
\depone{}(\coq{m}) = \{\coq{m_1, m_2, .., m_n}\} and \depone{}(\coq{m_n})
= \{\coq{m_i1, .., m_ik}\}.  \depone{} is transitively closed since
records are well typed in the empty context and describes a DAG since
Coq does not admit circular definitions.
\end{definition}

\begin{definition}[\factories{} \F{}, \mixinbuilders{} \from{}(f,m) and functions
   \provides{} and \requires{}]
  A mixin builder \(mb \in \from{}(f,m)\) is any term which return type is a
  mixin \(m\), and which parameters are the carrier
  type \coq{(T: Type)}, an arbitrary number of \mixins{} and an
  additional argument \coq{f} called \factory{}, which may depend on
  the \mixins{}.  Eg.
\begin{coqcode}
Definition mb (T : Type) (p_1 : m_1 T) (p_2 : m_2 T p_1) .. (p_n : m_n T p_i1 .. p_ik)
   (x : f p_1 .. p_n) : m p_j1 .. p_jl := ...
\end{coqcode}
We have the following functions \begin{itemize}
    \item\( \requires{} \in \F{} \to \powerset\M{} \)
    \item\( \provides{} \in \F{} \to \powerset\M{} \)
    \item\( \from{} \in \F{} \times \M{} \to \Lang{} \)
  \end{itemize}
  We call \(\from{} (f,m) = mb\) the \mixinbuilder{} that builds the mixin \(m\)
  from the factory \(f\).
  We call \(\requires{}(\coq{f}) = \{\coq{m_1, m_2, .., m_n}\}\) the set of
  mixins a factory depends on, thus requiring all the \mixinbuilders{}
  of a given factory to have the same set of dependency.  We call
  \(\provides{}(\coq{f}) = \{m | \from{}(f,m) \textrm{ is defined}\}\) the set
  of mixins that a factory provides, through its \mixinbuilders{}.
...
Mixins are declared by the users as the fundamental building blocks of
a hierarchy. Factories produce various mixins, in particular a mixin is
a factory producing exactly one mixin. 

\begin{proposition}[\( \M{} \subseteq \F{} \)]
  where \(\requires{}(m) = \depone{}(m)\), \(\provides{}(m) = \{m\} \) and
  \(from(m,m) = \coq{fun T p_1 .. p_n (x : m T p_1 .. p_n) => x)}\)

\end{proposition}


\begin{definition}[\FB{}, factory builders]
A factory builder \(fb \in \FB{}\) is a notation which automatically finds
the mixin instances a factory depends on and takes as explicit argument
the contents of the factory. E.g.
\begin{coqcode}
Notation fb T x_1 .. x_n := (...).
\end{coqcode}
\end{definition}
The conception of such notation relies on unification with phantom types \cite{DBLP:conf/itp/MahboubiT13}.

\begin{command}[hb.declare_mixin M T fs.
                Record axioms_ := { .. }.
                hb.end]
...
new record both as a mixin and a factory
 It also creates a \factorybuilder{} \coq{M.Axioms}
\end{command}

\begin{command}[hb.declare_factory M T fs. Record axioms_ := { .. }. hb.instance bs. hb.end]
Creates a module \coq{M} with a record \coq{M.axioms_}, which depends
on the mixins \(\bigcup_{f \in \coq{fs}} \provides{}(f)\), and registers this
new record as factory, and \coq{bs} as \mixinbuilders
\end{command}
...
\coq{T} and, if needed, any number of other fields. E.g.
\begin{coqcode}
Record c (T : Type) := { p_1 : m_1 T; p_2 : m_2 T p_1; .. ; p_n : m_n T p_i1 .. p_ik}.
\end{coqcode}
...
\begin{command}[hb.structure fs] crafts a class $c \in \C{}$ where $\cdef{}(c) = HELP$ and its companion $s \in \Str{}$.
\end{command}

\begin{command}[hb.instance c fis]
