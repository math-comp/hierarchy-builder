...
It is based on a disciplined use of records and projections and on the extension
mechanism of Canonical Structures.

\subsection{Describing a structure}
...
\subsection{Linking structures and inferring instances}
...
TODO: Example of a by had flatten class, maybe the one of the join.
...
them via dedicated APIs. For example the API for declaring an inductive type
will generate a value of type \elpi{@inductive} that would be printed
as \elpi{«nat»}.
...
arguments of a predicate, while \elpi{external} signal the implementation is
in OCaml (rather than $\lambda$Prolog), that is the predicate is a builtin one.
...
fields (it is handy to have them in the rest of the code). TODO: @factoryname -> @classname
...
of a record automatically generated by Coq.
The last detail worth discussing is that this program makes no use of
backtraking and it signat it by using the \elpi{std.do!} builtin.
...
effect, is also made available by Coq-Elpi via a dedicated API to extend
data bases. A data base is a named set of clauses and
commands typically are built by accumulating specific clauses on top
...
is Elpi code embedded in a Coq file, and all the vernacular commands beginning
