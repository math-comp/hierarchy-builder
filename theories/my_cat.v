...
Proof.
(* move: F G => [F [[/= Fhom]]] [G [[/= Ghom]]] in eqFG *. *) 
 destruct F as [F [[Fhom]]]. 
 destruct G as [G [[Ghom]]]. 
 simpl in eqFG; simpl.
(* 
 have -> : eqFG = (fun x => congr1 (@^~ x) (funext eqFG)).  
 by apply/funext => x; rewrite congr1_funext.
*)
 assert (eqFG = (fun x => congr1 (@^~ x) (funext eqFG))) as A.
 { eapply funext; intro. rewrite congr1_funext; auto. }
 rewrite A; clear A.
(* move: (funext eqFG) Ghom => {eqFG}. *)
 generalize (funext eqFG); intro funext. 
 revert Ghom.
(*  case: _ / => Ghom /= eqFGhom. *)
 destruct funext; simpl.
 intros Ghom eqFGhom.
(* congr PreFunctor.Pack. 
   congr PreFunctor.Class; congr IsPreFunctor.Axioms_. *)
 repeat f_equal. 
(*  by do 3!apply: funext=> ?. *)
 repeat (eapply funext; intro); auto.
Qed.
